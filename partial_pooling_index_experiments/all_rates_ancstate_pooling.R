...
model {
  ...
  beta ~ gamma(1,1);
  for (d in 1:D) {
    target += pruning(N,B,child,parent,brlen,tiplik[,((2*d)-1):(2*d)],alpha,beta);
  }
}
generated quantities {
  real log_lik[D];
  for (d in 1:D) {
    log_lik[d] = pruning(N,B,child,parent,brlen,tiplik[,((2*d)-1):(2*d)],alpha,beta);
  }
}"
...
for (t in inds) {
  ...
  save.image('all_rates_pooling.Rdata')
}